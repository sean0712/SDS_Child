...
public class Invest_DB_Control {
    ...
    public void Add_List_information(int trade_type, int trade_price, int trade_amount, float trade_gain, int trade_count, String trade_day) {
        int zero = 0;
        Invest_List_DB list_db = new Invest_List_DB(trade_type, trade_price, trade_amount, trade_gain, trade_count, trade_day);
        databaseReference.child("invest_list").child(User.stdNum).child(Get_Time2()).child("total_trade_count").setValue(zero);
    }

    //이건 문제생길듯
    //하루에 한번 basic data 를 생성하는 메서드
    public void Day_update() {
        databaseReference.child("invest_std_information").child(User.stdNum).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                Invest_User_DB std;
                std = dataSnapshot.getValue(Invest_User_DB.class);
                String last_update = std.getLast_update();

                //오늘 처음 접속했다면 주식 db업데이트
                if (last_update == null || !last_update.equals(Get_Time2())) {
                    //Add_basic_information(Get_Time2(), 0, 0, 0); //이부분은 다른데로 빼야 할듯
                    std.setLast_update(Get_Time2());
                    databaseReference.child("invest_std_information").child(User.stdNum).setValue(std);

                    //오늘 자 리스트 추가
                    Add_List_information(0, 0, 0, 0, 0, Get_Time2());
...
//    }
                ...
                databaseReference.child("day_information").child(Get_Time2()).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        if (have_miso < day_price * want_buy_amount) {
                            ...
                            //거래기록 리스트 추가
                            Add_List(0, day_price, want_buy_amount, 0, Get_Time2());
                ...
                databaseReference.child("day_information").child(Get_Time2()).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        if (have_Invest < want_sell_amount) {
                            ...
                            // 여기에 리스트관련 작업해야함
                            Add_List(1, day_price, want_sell_amount, gain_calc, Get_Time2());
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public void Add_List(int trade_type, int trade_price, int trade_amount, float trade_gain, String trade_day) {

        databaseReference.child("invest_list").child(User.stdNum).child(Get_Time2()).child("total_trade_count").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                int total_trade_count = dataSnapshot.getValue(Integer.class);
                total_trade_count++;
                databaseReference.child("invest_list").child(User.stdNum).child(Get_Time2()).child("total_trade_count").setValue(total_trade_count);

                Invest_List_DB list_db = new Invest_List_DB(trade_type, trade_price, trade_amount, trade_gain, total_trade_count, trade_day);
                String trade_count = String.valueOf(list_db.getTrade_count());
                databaseReference.child("invest_list").child(User.stdNum).child(Get_Time2()).child(trade_count).setValue(list_db);
            }
